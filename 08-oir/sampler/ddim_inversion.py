...
from configs.basic_config import GUIDANCE_SCALE, device, NUM_DDIM_STEPS
...
class DDIMInversion:
    ...
    def reinversion(
        ...
        origin_embedding = origin_embedding.repeat(
            latent.shape[0], 1, 1
        )  # 和之前的inversion不同，要对每一个样本都设置一个prompt，原来只要一个inverison就可以用到所有的上面
        latent = latent.clone().detach()
    ...
    def invert(
        ...
        image_gt = basic_utils.load_512(image_path, *offsets)
        image_rec, ddim_latents = self.ddim_inversion(
            image_gt,
        )
        return ddim_latents
